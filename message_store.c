...
char *slots_plan() {
	...
	static char slots_array[SLOTS_AMOUNT+1];


        void fill_slots(char *out_slots_array,char *in_slot_list, char token) {
		int r;
		int out_index;

		// call to this function with ("........................","ABEF",'5') should result in "55..55................."
		// so a 5 will be filled in in the positions indicated by A,B,E and F

		r=0;
		while (in_slot_list[r] != '\0') {
			out_index = (int) (in_slot_list[r] -'A'); 
			if( (out_index >= 0) && (out_index < SLOTS_AMOUNT) ) {
				out_slots_array[ (int) (in_slot_list[r] -'A') ]=token;
			}
			r++;
 		}
  	}
	...
	slots_array[SLOTS_AMOUNT]='\0';
        ...
        sqlite3_prepare_v2(db, "select value from config where tag='stations490' ", -1, &stmt, NULL);
        if (sqlite3_step(stmt) != SQLITE_DONE) {
                        if (sqlite3_column_type(stmt, 0))
                        {
				tag_text =(char*) sqlite3_column_text(stmt, 0);
				fill_slots(slots_array,tag_text,'4');
                        }
        }
        sqlite3_finalize(stmt);  //also frees pointer returned by sqlite3_column_text


        sqlite3_prepare_v2(db, "select value from config where tag='stations518' ", -1, &stmt, NULL);
        if (sqlite3_step(stmt) != SQLITE_DONE) {
                        if (sqlite3_column_type(stmt, 0))
                        {
				tag_text =(char*) sqlite3_column_text(stmt, 0);
				fill_slots(slots_array,tag_text,'5');
                        }
        }
        sqlite3_finalize(stmt);  //also frees pointer returned by sqlite3_column_text
